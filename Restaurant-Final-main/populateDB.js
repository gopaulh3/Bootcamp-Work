...
async function loadAndInsert() { 
    const restaurantFile = './restaurantData.json'; //asynchronously reads the contents of the restaurant data file
    ...
    for (let i = 0; i < restaurantsArray.length; i++) {
        ...
        for (let j = 0; j < currentRestaurant.menus.length; j++) {
            ...
            for (let k = 0; k < currentMenu.items.length; k++) {
                ...
                await MenuItem.create ({item: currentMenuItem.name, price: currentMenuItem.price, menu_id: menuCounter})
            }
